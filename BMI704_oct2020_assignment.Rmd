...
Today, you're going to be working with a subset of the data from the Qin paper to reproduce this result to show how microbes can be associated with either disease status or drug usage or both. Here are the three steps in this brief tutorial:
...
metadata <- readRDS(url("","rb"))
abundance <- readRDS(url("","rb"))
...
Based on the dimension of the metadata dataframe, we have 140 unique subjects. We have 7 columns in our metadata, one corresponding to a subject ID, one "study_condition" column that indicates whether an individual had hypertension (1) or not (0). The other columns correspond to the following data:
...
We will again do this for all ~600 microbial features in the dataset. 
...
full_association_output_multi = map(microbe_names, function(x) run_association(x,merged_data)) %>% bind_rows
...
colnames(full_association_output_subset) = c('microbial_feature_name','univariate_estimate_univariate','pvalue_adjusted_univariate')
...
all_association_output %>% filter(pvalue_adjusted_multivariate<0.05)
```
